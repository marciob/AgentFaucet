...
1. **Web app** — where human developers authenticate via GitHub or X, receive a reputation score based on their account history, get an ERC-8004 agent identity minted on their behalf, and receive a JWT token for their agents
2. **Backend relayer** — validates tokens, enforces rate limits per developer identity (not per wallet), mints ERC-8004 identities, posts reputation updates to the ERC-8004 Reputation Registry, and calls the on-chain faucet contract on the agent's behalf (so the agent never needs gas to claim)
...
┌───────────────────────▼──────────────────────────────────────┐
│                      BACKEND / API                           │
│  - OAuth callback → fetch profile + repos                    │
│  - Reputation scoring engine                                 │
│  - JWT issuance and validation                               │
│  - ERC-8004 Identity Registry interaction (mint agent NFT)   │
│  - ERC-8004 Reputation Registry interaction (post feedback)  │
│  - Rate limiting per developer identity                      │
│  - Relayer: calls FaucetPool to send tokens                  │
│  - Sponsor management                                        │
└───────────────────────┬──────────────────────────────────────┘
                        │
          ┌─────────────┼─────────────┐
          ▼             ▼             ▼
┌─────────────┐ ┌─────────────┐ ┌──────────────────┐
│ FaucetPool  │ │ ERC-8004    │ │ ERC-8004         │
│ (our        │ │ Identity    │ │ Reputation       │
│ contract,   │ │ Registry    │ │ Registry         │
│ BNB Testnet)│ │ (BNB's     │ │ (BNB's deployed  │
│             │ │ deployed    │ │ contract)        │
│             │ │ contract)   │ │                  │
└─────────────┘ └─────────────┘ └──────────────────┘
...
┌──────────────────────────────────────────────────────────────┐
│                    AGENT SKILL                               │
│  - claim.js   → POST /claim with JWT                        │
│  - status.js  → check allocation + ERC-8004 identity        │
│  - return.js  → return unused tokens                         │
│  - sponsors.js → list active campaigns                       │
...
- SQLite for hackathon (simple, no infra), upgradeable to PostgreSQL
- ethers.js v6 or viem for blockchain interaction
- passport.js for OAuth strategies

### 8.2 Endpoints

**Authentication:**
- GitHub OAuth initiation and callback — fetches profile and public repos, calculates reputation, mints ERC-8004 agent identity via relayer, issues JWT containing both developer identity and ERC-8004 token ID
- X/Twitter OAuth initiation and callback — same flow with X-specific signals

**Faucet operations (all require valid JWT in Authorization header):**
- Claim tokens: accepts a wallet address and optional amount. Validates JWT, checks daily rate limit for that developer identity, calls the faucet contract via the relayer wallet, returns the transaction hash, remaining daily allocation, and ERC-8004 agent token ID.
- Check status: returns the developer's reputation score, tier, daily limit, amount claimed today, amount remaining, ERC-8004 agent token ID, and on-chain reputation summary.
- Return tokens: records a token return event, credits the developer's reputation both internally and on the ERC-8004 Reputation Registry.

**Sponsor operations:**
- Create campaign: sponsor defines campaign name, description, target protocol
- List active campaigns: public, no auth required
- View campaign activity: shows which agents (by ERC-8004 ID) tested, interaction counts, reputation signals posted

**Public:**
- Global stats: total claims, unique developers, pool balance, active sponsor count, total ERC-8004 agent identities registered

### 8.3 JWT Token Contents
...
- Enforced at the backend level (fast, no gas cost)
- Every claim response includes remaining allocation so the agent can make informed decisions

### 8.5 Relayer Wallet

The backend holds a single testnet wallet that serves as the relayer. It is the only address authorized to call the claim function on the FaucetPool contract. It also performs ERC-8004 interactions (minting agent identities, posting reputation). Its private key is stored in an environment variable on the server. It needs to be funded with testnet BNB for gas — initially from the official BNB faucet. Monitor its balance and alert when low.

### 8.6 ERC-8004 Reputation Posting Logic

The backend runs a background process (or triggers after claim events) that:
1. Monitors the chain for on-chain activity from wallets that received tokens via AgentFaucet
...
4. Updates the internal reputation score which may adjust the developer's tier for future claims
...
React or Next.js with Tailwind CSS. Chart library for statistics visualization. wagmi/viem or ethers.js for wallet interactions (sponsor deposits only).
...
1. FaucetPool smart contract deployed on BNB Testnet
2. Backend with GitHub OAuth, reputation scoring, JWT issuance, claim endpoint with rate limiting, and relayer logic
3. ERC-8004 Identity Registry integration: mint agent identity NFT during OAuth registration
4. Working Skill with claim and status scripts
5. Minimal frontend: landing page with GitHub auth, dashboard showing score, ERC-8004 agent ID, and token copy
